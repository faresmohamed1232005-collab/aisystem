@extends('layouts.app')
@section('title', 'إضافة مريض مؤمّن')
...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <div class="flex items-center gap-3 mb-6">
            ...
            <h2 class="text-xl font-bold text-gray-800">إضافة مريض مؤمّن عليه</h2>
        </div>
        ...
        <form action="{{ route('insured-patients.store') }}" method="POST" class="space-y-4">
            @csrf
            @include('insured-patients._form', ['insuredPatient' => null])
            <div class="flex gap-3 pt-2">
                <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold px-6 py-3 rounded-xl transition flex items-center gap-2">
                    <i class="fas fa-user-plus"></i> حفظ
                </button>
