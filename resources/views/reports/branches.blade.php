...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm p-5">
            <h3 class="font-bold text-gray-800 mb-4"><i class="fas fa-chart-column text-indigo-500 ml-1"></i> مقارنة الفروع (إيراد وربح)</h3>
            @if($branches->isEmpty())
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div class="p-4 border-b font-bold text-gray-800"><i class="fas fa-code-branch text-indigo-500 ml-1"></i> تفصيل الفروع</div>
        <div class="overflow-x-auto">
            <table class="w-full text-sm min-w-[720px]">
                <thead class="bg-gray-50 text-gray-500 text-right">
                    <tr>
                        <th class="px-4 py-3 font-medium">الفرع</th>
                        <th class="px-4 py-3 font-medium">الإيراد</th>
                        <th class="px-4 py-3 font-medium">الربح</th>
                        <th class="px-4 py-3 font-medium">الهامش</th>
                        <th class="px-4 py-3 font-medium">الفواتير</th>
                        <th class="px-4 py-3 font-medium">وحدات المخزون</th>
                        <th class="px-4 py-3 font-medium">قيمة المخزون</th>
                        <th class="px-4 py-3 font-medium">تحت الحد</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-4 py-3 font-semibold text-gray-800">{{ number_format($b['revenue'],2) }}</td>
                        <td class="px-4 py-3 text-green-600">{{ number_format($b['profit'],2) }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ number_format($b['margin'],1) }}%</td>
                        <td class="px-4 py-3 text-gray-500">{{ number_format($b['invoices']) }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ number_format($b['inv_units']) }}</td>
                        <td class="px-4 py-3 text-gray-500">{{ number_format($b['inv_value'],2) }}</td>
                        <td class="px-4 py-3">
                            @if($b['low_stock']>0)
                    ...
                    @empty
                    <tr><td colspan="8" class="text-center py-12 text-gray-300"><i class="fas fa-code-branch text-4xl block mb-2"></i>لا توجد فروع أو بيانات</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div class="grid grid-cols-2 gap-3">
                @php
                    $tBadge = [
                        'draft'    => ['مسودّة', 'bg-gray-100 text-gray-700'],
                        'approved' => ['معتمدة', 'bg-amber-100 text-amber-700'],
                        'sent'     => ['مُرسَلة', 'bg-blue-100 text-blue-700'],
                        'received' => ['مستلمة', 'bg-green-100 text-green-700'],
                        'rejected' => ['مرفوضة', 'bg-red-100 text-red-700'],
                    ];
                @endphp
                @foreach($transferCounts as $status=>$count)
                @php($tb = $tBadge[$status] ?? [$status, 'bg-gray-100 text-gray-700'])
                <div class="rounded-xl p-3 {{ $tb[1] }}">
                    <div class="text-xs opacity-80">{{ $tb[0] }}</div>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-5">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="rounded-xl p-3 bg-purple-100 text-purple-700">
                    <div class="text-xs opacity-80">مبيعات التأمين (الفترة)</div>
                    <div class="text-lg font-bold">{{ number_format($insuranceSales->covered ?? 0,2) }}</div>
                    <div class="text-xs opacity-70">{{ (int)($insuranceSales->invoices ?? 0) }} فاتورة</div>
                </div>
                <div class="rounded-xl p-3 bg-amber-100 text-amber-700">
                    <div class="text-xs opacity-80">مطالبات مُرسَلة (مديونية)</div>
                    <div class="text-lg font-bold">{{ number_format(optional($claimsByStatus->get('submitted'))->amount ?? 0,2) }}</div>
                    <div class="text-xs opacity-70">{{ optional($claimsByStatus->get('submitted'))->c ?? 0 }} مطالبة</div>
                </div>
            </div>
